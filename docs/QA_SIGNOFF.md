...
### New Testament, Wave 1 (worlds 40-43, the four Gospels) — verbatim QA complete, 2026-09-04

First New Testament books to clear this gate. All four checked against Bible Hub (biblehub.com/web/),
a source not already used for text verification during drafting/review (ebible.org + BibleGateway).

| 40. Matthew | 68 | `pass-with-corrections` | Two label-only fixes (3:13-15 and 9:6 were mislabeled `(WEB, trimmed)` when both are complete, unabridged verses — corrected to `(WEB)`); one apparent discrepancy at 7:24 ("a rock" vs. "the rock") investigated and resolved in the brief's favor after triangulating two more sources; 27:24-25/8:11-12 and the Olivet Discourse (chs. 24-25) exclusions confirmed absent via grep | `docs/qa/matthew-verbatim-qa.md` | 2026-09-04 |
| 41. Mark | 45 | `pass-with-corrections` | **Real corrections:** 2:9-10 and 5:41-42 each silently dropped a trailing narrator clause with no trim label — both restored with the trim honestly marked, no wording changed; 16:9-20 (the Longer Ending) and ch. 13 (Olivet Discourse) exclusions confirmed absent via grep | `docs/qa/mark-verbatim-qa.md` | 2026-09-04 |
| 42. Luke | 59 | `pass-with-corrections` | **Real correction:** 3:21-22 had a stray comma not present in source, removed; 16:19-21 silently dropped its final clause with no trim label — restored with the trim honestly marked; ch. 21 and 22:43-44 exclusions confirmed absent, 23:34 confirmed limited to its first sentence only, and all six named recall-only verses (1:32-33, 1:34-35, 3:21-22, 4:21, 22:19-20, 23:42-43, 24:5-6) confirmed still recall-only via grep | `docs/qa/luke-verbatim-qa.md` | 2026-09-04 |
| 43. John | 46 | `pass-with-corrections` | Three label-only fixes (4:9, 4:25-26, 9:25 were unlabeled silent truncations — now labeled `(trimmed)`) plus one incorrect trim-label removed (6:11, already the complete verse) and **one real wording correction:** 14:2-3 read "I go to prepare a place for you," which doesn't appear verbatim in WEB — corrected to "I am going to prepare a place for you," confirmed against two independent sources. Every named exclusion (John 7:53-8:11, 3:5, 9:8-41, "the Jews" language, 6:51-58) confirmed absent via grep, and the six divinity-claim verses (1:1, 1:3, 1:14, 8:58, 10:30, 20:28) confirmed to appear only as `recall` items, never `multiple_choice` — the exact condition Kachi's 2026-09-04 resolution of this book's escalation depends on holding. | `docs/qa/john-verbatim-qa.md` | 2026-09-04 |
